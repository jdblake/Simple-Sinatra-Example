...
This gives you a <em>lot</em> of control over the db. You load a file into <pre>irb</pre> using the
<pre>-r</pre> switch:
...
You will get <pre>false</pre> because the validation failed.
...
feedback about it lie you do when you go the <pre>new/save</pre> route.

2. If you had tried this:

     Student.create(:name => "Jon", :grade => "4")

the record <u>would</u> be saved because the string <pre>"3"</pre> can be implicitly converted to an integer
value
